...
var FormulaController = BaseController.extend({
    ...
    _handleOnDeleteFormulaInstance: function(formulaName) {
        ...
        for(var i = 0; i < me.$scope.formulas.length; i++) {
            ...
            if(formula.name == formulaName) {
                angular.element(document.getElementById("instance-"+formula.id)).removeClass('highlightingElement');
                if(!me.$scope.multipleInstance) {
    ...
    _handleLoadFormulaInstances: function(formulaTemplate, httpResult) {
        var me = this;
        var formulaInstances = httpResult.data? httpResult.data: [];
            ...
            if (typeof formulaAppConfig === 'string') {
                ...
                for(var j = 0; j < formulaTemplates.length; j++) {
                    ...
                    if(formulaAppConfig.name === formulaTemplate.name) {
                        ...
                        if (!me._cloudElementsUtils.isEmpty(formulaAppConfig.instanceName)) {
                            formulaTemplate.instanceName = formulaAppConfig.instanceName;
                        }

                        if (!me._cloudElementsUtils.isEmpty(formulaAppConfig.notificationEmail)) {
                            formulaTemplate.notificationEmail = formulaAppConfig.notificationEmail;
                        }
